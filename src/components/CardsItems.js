...
const CardsItems = () => {
  ...
  resinfo.cards[5]?.groupedCard?.cardGroupMap?.REGULAR?.cards[2]?.card?.card?.categories?.[0]?.itemCards ||
    ...
    <div className="p-10 md:p-20 ">
      ...
      <div className="mt-12 ">
        ...
        <ul className="flex flex-col">
          {item.map((item) => {
            const { name, imageId, description, defaultPrice } =
              item?.card?.info;
            const { vegClassifier } = item?.card?.info?.itemAttribute;
            return (
              <div
                key={item?.card?.info?.id}
                className="flex  flex-col md:flex-row justify-center items-center md:justify-start mt-16"
              >
                <img
